...
class NAV_Analyzer(Analyzer):
    ...
    def analyze(self):
        ...
        self.NAV_by_ticker = self.NAV.iloc[-1]
    ...
    def display(self, benchmark_nav):
        """
        Graphs of portfolio NAV by time and by ticker.
        """
        ...
        plt.plot(self.NAV_by_date.index, self.NAV_by_date.values, marker='o', linestyle='-', markersize=5, label = 'User Portfolio')
        plt.plot(self.NAV_by_date.index, benchmark_nav.NAV_by_date.values, marker='x', linestyle = '-', markersize=5, label='S&P 500')
        ...
        st.pyplot(plt)
        plt.clf()
        st.write("Final Net Asset Value (NAV) by ticker:")
        plt.figure(figsize=(12, 8))
        plt.bar(self.NAV_by_ticker.index, self.NAV_by_ticker.values)

        plt.xlabel('Ticker')
        plt.ylabel('NAV')
        plt.title('User Portfolio: Final Net Asset Value (NAV) by ticker')
        plt.tight_layout()
        st.pyplot(plt)
